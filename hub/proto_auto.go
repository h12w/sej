...
func (o *Request) MarshalLen() (int, error) {
	...
	if x := len(o.ClientID); x != 0 {
		if x > ColferSizeMax {
			return 0, ColferMax(fmt.Sprintf("colfer: field hub.Request.ClientID exceeds %d bytes", ColferSizeMax))
		}
		for l += x + 2; x >= 0x80; l++ {
			x >>= 7
		}
	...
	if x := len(o.JournalDir); x != 0 {
		if x > ColferSizeMax {
			return 0, ColferMax(fmt.Sprintf("colfer: field hub.Request.JournalDir exceeds %d bytes", ColferSizeMax))
		}
		for l += x + 2; x >= 0x80; l++ {
			x >>= 7
		}
	...
	if l > ColferSizeMax {
		return l, ColferMax(fmt.Sprintf("colfer: struct hub.Request exceeds %d bytes", ColferSizeMax))
	}
	return l, nil
}
...
func (o *Request) Unmarshal(data []byte) (int, error) {
	...
	if header == 2 {
		...
		if x > uint(ColferSizeMax) {
			return 0, ColferMax(fmt.Sprintf("colfer: hub.Request.ClientID size %d exceeds %d bytes", x, ColferSizeMax))
		}
	...
	if header == 3 {
		...
		if x > uint(ColferSizeMax) {
			return 0, ColferMax(fmt.Sprintf("colfer: hub.Request.JournalDir size %d exceeds %d bytes", x, ColferSizeMax))
		}
...
eof:
	if i >= ColferSizeMax {
		return 0, ColferMax(fmt.Sprintf("colfer: struct hub.Request size exceeds %d bytes", ColferSizeMax))
	}
	return 0, io.EOF
}
